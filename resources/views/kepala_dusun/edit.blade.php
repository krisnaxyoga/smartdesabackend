        ...
        <div class="row">
            <div class="col-md-6">
                <div class="box">
                    ...
                    <div class="box-body">
                        <form method="POST" action="{{ url('kepala-dusun/'.$data->id) }}">
                            {{ csrf_field() }}
                            @method('PUT')
                            <div class="form-body">
                                @if ($errors->any())
                                ...
                                <div class="form-group">
                                    <label>Nama</label>
                                    <input name="name" required type="text" class="form-control m-input" placeholder="Nama" value="{{ $data->name }}">
                                </div>
                                {{-- <div class="form-group">
                                    <label>Username</label>
                                    <input name="username" required type="text" class="form-control m-input" placeholder="Username" value="{{ $data->username }}">
                                </div> --}}
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="dusun_id" id="" required class="form-control">
                                        @foreach($dusun as $item)
                                            <option value="{{$item->id}}" {{$item->id == $data->dusun_id ? 'selected' : ''}}>{{$item->dusun}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                {{-- <div class="form-group">
                                    <label>PIN</label>
                                    <input name="pin"  type="password" class="form-control m-input" placeholder="Pin">
