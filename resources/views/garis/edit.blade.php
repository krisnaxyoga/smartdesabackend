  ...
  <div class="padding">
    <div class="row">
      <div class="col-lg-12">
        ...
        @endif
        <div class="box">
          <div class="box-header"><h3>Form Garis</h3></div>
          <div class="box-divider m-0"></div>
          <form class="form-horizontal" action="/peta/garis/{{ $data->id }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="box-body">
              <div class="row">
                <div class="col-lg-6">
                  <div class="form-group row">
                    <label class="col-form-label col-lg-3">Nama Garis</label>
                    <div class="col-lg-9">
                  ...
                  <div class="form-group row">
                    ...
                    <div class="col-lg-9">
                      <select name="tipe_garis_id" class="form-control">
                        <option value="">Pilih kategori...</option>
                        @foreach ($types as $type)
                          <option value="{{ $type->id }}" {{ (old('tipe_garis_id') ?: $data->tipe_garis_id) == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                        @endforeach
                      </select>
                    </div>
                  </div>
                  ...
                  <div class="form-group row">
                    ...
                    <div class="col-lg-9">
                      ...
                      <p class="mb-1"><img src="{{ $data->photo ?: '/images/dummy-landscape.png' }}" width="240" id="photo" /></p>
            ...
            <div class="box-footer">
              ...
              <a href="/peta/garis" class="btn btn-rounded btn-default">Batal</a>
            </div>
          </form>
        </div>
      </div>
...
<script type="text/javascript">
  let $photoInput = $('[name="photo"]'),
    ...
    coordinates = {!! json_encode(old('coordinates') ?: $data->coordinates) !!},
    polylineOptions = {
      strokeColor: "#FF0000",
      strokeWeight: 5,
      editable: true
  ...
  $('input[name="coordinates"]').val(JSON.stringify(coordinates))
  ...
  function initMap() {
    ...
    if (coordinates.length > 0) {
      selectedShape = new google.maps.Polyline(polylineOptions);
      ...
      google.maps.event.addListener(selectedShape.getPath(), 'insert_at', function(){
        console.log(selectedShape.getPath().getArray())
        $('input[name="coordinates"]').val(JSON.stringify(selectedShape.getPath().getArray()));
      });

      google.maps.event.addListener(selectedShape.getPath(), 'remove_at', function(){
        console.log(selectedShape.getPath().getArray())
        $('input[name="coordinates"]').val(JSON.stringify(selectedShape.getPath().getArray()));
      });
